...
- [ ] `fvm flutter analyze && fvm flutter test -r compact` をローカルで実行し、Flutter側の静的解析とテストを確認する
- [ ] Androidエミュレータを再起動し、`pnpm android` で手動動作確認を行う
- [ ] すべて確認後にコミットと push を実施する
...
- [x] 差分抽出
  ...
  - [x] 二値化 & 連結成分解析
- [x] 比較パイプラインの高精度化
  - [x] 整列を 384×384 で実施し、整列後に範囲指定を適用する
  - [x] 矩形領域のみを 256×256 に再サンプリングして差分計算する
  - [x] 左画像の特徴点・記述子を画像選択時に前計算し、検出ごとに再利用する
...
- [x] 検出の最小面積を割合基準に変更（既定 5%）
  - [x] 仕様反映: `prompt_spec.md` に最小面積(%) を追記（64×64空間比率で判定）
  - [x] テスト追加（Dart）: 5%未満の領域は検出されず、5%以上は検出される
  - [x] 実装: `Settings` に `minAreaPercent` を追加（初期値 5）。検出器で `ceil(percent/100 * width * height)` を最小面積に適用
  - [x] 比較画面・結果表示は既存のまま（検出器の出力に依存）
