...
Future<Uint8List> _makePng({int w = 64, int h = 64, bool withDiff = false}) async {
  ...
  if (withDiff) {
    ...
    final rect = Rect.fromLTWH(w * 0.35, h * 0.35, w * 0.3, h * 0.3);
...
void main() {
  testWidgets('異なる画像を与えると検出が行われ、件数がSnackBarに表示される', (tester) async {
    ...
    // ComparePage には常時アニメがあるため、pumpAndSettle は避けて段階的に待機
    for (var i = 0; i < 20; i++) {
    ...
    expect(
      find.byWidgetPredicate((w) => w is SnackBar &&
          ((w.content is Text) && (w.content as Text).data!.contains('検出数: '))),
      findsOneWidget,
    );
  });
}
