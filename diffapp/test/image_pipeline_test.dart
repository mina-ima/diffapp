...
void main() {
  group('calculateResizeDimensions', () {
    ...
    test('throws on non-positive inputs', () {
      ...
      expect(() => calculateResizeDimensions(100, 100, targetMaxWidth: 0),
          throwsArgumentError);
    });
  });
  ...
  group('scaleRectForResizedImage', () {
    test('returns same rect when no resize needed', () {
      final rect = const IntRect(left: 10, top: 20, width: 30, height: 40);
      final scaled = scaleRectForResizedImage(rect, 800, 600, targetMaxWidth: 1280);
      expect(scaled, rect);
    });

    test('scales rect proportionally on downscale', () {
      // Original: 4000x3000, target width 1280 -> scale 0.32
      final rect = const IntRect(left: 200, top: 150, width: 400, height: 300);
      final scaled = scaleRectForResizedImage(rect, 4000, 3000, targetMaxWidth: 1280);
      expect(scaled, const IntRect(left: 64, top: 48, width: 128, height: 96));
    });

    test('throws when rect is out of bounds', () {
      final rect = const IntRect(left: 3900, top: 2900, width: 200, height: 200);
      expect(
  ...
  group('nonMaxSuppression + iou', () {
    ...
    test('respects maxOutputs (top-K)', () {
      ...
      final kept = nonMaxSuppression(boxes, scores, iouThreshold: 0.0, maxOutputs: 20);
